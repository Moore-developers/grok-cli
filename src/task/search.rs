...
const DEFAULT_X_SEARCH_MODEL: &str = "grok-4.6";
...
mod tests {
    ...
    fn parse_x_search_response_extracts_answer_and_citations() {
        ...
        let parsed = parse_x_search_response(&opts, "grok-4.6", &upstream).unwrap();
    ...
    fn parse_x_search_response_rejects_missing_answer() {
        ...
        let error = parse_x_search_response(&opts, "grok-4.6", &upstream).unwrap_err();
